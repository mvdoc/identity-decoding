...
programname=$0
function usage {
    echo "usage: $programname subjectid metric task hemi [deconvolve|remlfit]"
    exit 1
}
...
OUTDIR=$BASEDIR/derivatives/slcvrsa-blockrun-"$DECON"-fsaverage6
...
CMD="$BASEDIR/scripts/02mvpa/run_sl_cvrsa.py \
     -i $INPUT \
     --metric $METRIC \
     -f $FSDIR \
     --mask $MASK \
